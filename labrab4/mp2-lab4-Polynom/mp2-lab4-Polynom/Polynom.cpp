...
Polynom::Polynom(string str)
{
	...
	{
		symbol = str[i];
		if (!((symbol == '*') || (symbol == '^') || (symbol == '+') || (symbol == '-') || (symbol == 'x') || ((symbol >= '0') && (symbol <= '9'))) || (symbol == ' '))
			throw"Введён недопустимый символ";
	}
	List* listStart = new List;
	listStart->monom.power = -1;
	listStart->monom.coef = 0;
	List* actual_part_list = listStart;
	i = 0;
	while (i < size)
	{
		j = i;
		...
		tmpstr = str.substr(i, count);
		j = 0;
		actual_part_list->pNext = new List;
		actual_part_list = actual_part_list->pNext;
		actual_part_list->monom.coef = 1;
		if (tmpstr[j] == '-')
		{
			actual_part_list->monom.coef = -1;
			j++;
		...
		if (tmpstr[j] == '+')
			j++;
		if ((tmpstr[j] >= '0') && (tmpstr[j] <= '9'))
			actual_part_list->monom.coef *= PartOfStringToInt(tmpstr, j);
		else if (tmpstr[j] == 'x');
		else
			throw"Ошибка ввода полинома: Неверно начат полином";
		if (tmpstr[j] == '*')
			j++;
		actual_part_list->monom.power = 0;
		if (tmpstr[j] == 'x')
		{
			j++;
			if (tmpstr[j] == '^')
			...
			if ((tmpstr[j] >= '0') && (tmpstr[j] <= '9'))
				actual_part_list->monom.power += maxPower * maxPower*PartOfStringToInt(tmpstr, j);
			else
				actual_part_list->monom.power += maxPower * maxPower * 1;
		}
		if (j != tmpstr.size())
			throw"Ошибка ввода полинома: Неправильная запись монома 4";
		i += j;
	}
	actual_part_list->pNext = listStart;
	this->pHead = listStart;
	this->Sort_MaxPower();
}
...
Polynom::Polynom(const Polynom &polynom)
{
	...
	List* tmp = new List;
	List* start_tmp = tmp;
	List* actual_list = polynom.pHead;
	actual_list = actual_list->pNext;
	if (actual_list->monom.power == -1)
	{
		start_tmp->pNext = this->pHead;
	}
	while (actual_list->monom.power != -1)
	{
		tmp->pNext = new List;
		tmp = tmp->pNext;
		tmp->monom.coef = actual_list->monom.coef;
		tmp->monom.power = actual_list->monom.power;
		actual_list = actual_list->pNext;
	}
	this->pHead->pNext = start_tmp->pNext;
...
Polynom Polynom::operator*(const Polynom& polynom)
{
	...
	//int count = 0;
	//Polynom tmpPolynom1;
	//List* result1 = tmpPolynom1.pHead;
	//List* Pol3Link = tmpPolynom.pHead;
	//Pol3Link = Pol3Link->pNext;
	//List* Pol4Link;
	//while (Pol3Link->monom.power != -1)
	//{
	//	
	//	Pol4Link = Pol3Link;
	//	Pol4Link = Pol3Link->pNext;
	//	while (Pol4Link->monom.power != -1)
	//	{
	//		if (Pol4Link->monom.power == Pol3Link->monom.power )
	//		{
	//			if ((Pol3Link->monom.coef + Pol4Link->monom.coef) == 0)
	//			{
	//				Pol3Link = Pol3Link->pNext;
	//				Pol4Link = Pol4Link->pNext;
	//				continue;
	//			}
	//			result1->pNext = new List;
	//			result1 = result1->pNext;
	//			result1->monom.power = Pol4Link->monom.power;
	//			result1->monom.coef = Pol3Link->monom.coef + Pol4Link->monom.coef;
	//			/*tmpPolynom1.pHead[i].monom.coef=0;
	//			Pol3Link[i].monom.coef = 0;*/
	//			Pol4Link = Pol4Link->pNext;
	//		}
	//		i++;
	//	}
	...
	//	Pol3Link = Pol3Link->pNext;
	//}
	//result1->pNext = tmpPolynom1.pHead;
	//
	return tmpPolynom;
